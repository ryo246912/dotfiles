...
- `dot_copilot/mcp-config.json` は `rulesync generate --targets copilotcli --features mcp --global` で直接生成する snapshot です。
...
- 2026-04-01 時点でも `rulesync 7.23.0` は `copilotcli` target で `command` を持つ stdio server を前提にします。
- この repo では Notion / deepwiki の remote MCP を `npx -y mcp-remote@latest <url>` として source に定義し、rulesync 単体で `dot_copilot/mcp-config.json` を生成します。
