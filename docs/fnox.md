...
[fnox](https://github.com/jdx/fnox) は secret を `fnox.toml` で宣言的に管理し、shell / CI から環境変数として注入するツールです。
...
- `fnox` の強みは **provider を差し替えても `fnox.toml` の構造と `fnox exec` / `fnox activate` という
  ...
  それに加えてリモート参照も同じ形式で扱える点が異なります。`dotenvx` は「`.env` という形式に強くこだわりがある」場合の、
  `sops` は「YAML/JSON の構造を保ったまま秘匿したい」場合の選択肢として残ります
...
`awlo`) と `dot_config/zabrze/fnox.toml` の `fnox` 系 (`fna` = activate, `fe` = exec, `fnv` = aws-vault
exec + fnox exec) を参照してください。`fe` は global snippet のため、コマンドラインの途中でも展開できます。

## セットアップ手順

### 1. Bitwarden Secrets Manager (`bitwarden-sm` provider, `bws`)
