...
dumpが長時間戻らない場合、まず進捗の行を見る。`0 lines`のままならserverがまだ最初の行を返していない（cursorを有効にしていると、CockroachDBではここで止まる）。行数が増えているなら単に遅いので、`ls -l`でfileが育っているかも合わせて確認する。
...
- identity列（`GENERATED ALWAYS AS IDENTITY`）を持つtableのINSERTには`OVERRIDING SYSTEM VALUE`を付ける。AgentsViewの`id`はこの形なので、付けないと`cannot insert into column "id"`でrestoreが止まる。idは他tableからの参照に使われるため、列を外して採番し直すことはできない。`GENERATED BY DEFAULT`の列には付けない。
- `psql`の`FETCH_COUNT`（cursorで受け取る件数）は`AGENTSVIEW_DUMP_FETCH_ROWS`で指定する。**既定は`0`（cursorを使わない）**である。CockroachDBのcursorではdumpが1時間経っても1行も返らず、cursorなしでは同じclusterから17 tableを書き出せているためである。cursorを使うと生成したINSERT文をclient memoryへ溜めずに済むので、実績のある相手では`AGENTSVIEW_DUMP_FETCH_ROWS=1000`のように指定して有効にできる。
