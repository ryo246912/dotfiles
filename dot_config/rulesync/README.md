...
```text
<repo root>/
├── rulesync.jsonc          # project-local config（repo root CLAUDE.md / .vscode/mcp.json）
├── .rulesync/
│   ├── mcp.json            # .vscode/mcp.json の source
│   └── rules/
...
- `<repo root>/.rulesync/mcp.json`
  - repo root の `.vscode/mcp.json`
...
### repo root の Copilot MCP だけ更新する

```sh
mise run rulesync-generate-mcp-copilot
# または直接
rulesync generate --targets copilot --features mcp
```
...
project-local の `.vscode/mcp.json` は repo 直下に生成されます。user-scope を更新した場合は、その後 `chezmoi re-add` で chezmoi source へ反映してください。
...
2. generate を実行する
   ...
   - repo root Copilot MCP: `mise run rulesync-generate-mcp-copilot`
...
4. `git diff -- CLAUDE.md .vscode/mcp.json dot_claude dot_codex dot_copilot dot_gemini dot_config/rulesync` で差分を確認する
...
- `copilot` の remote MCP は repo root `.rulesync/mcp.json` から `.vscode/mcp.json` を生成します。
- `copilotcli` の MCP は rulesync 7.23.0 では `command` を持つ stdio server のみ扱えるため、`dot_config/rulesync/.rulesync/mcp.json` には stdio subset だけを置いています。
