...
- `dot_config/rulesync/.rulesync/rules/COPILOT.md`
  - `~/.github/copilot-instructions.md`（generate 後に `chezmoi re-add`）
- `dot_config/rulesync/.rulesync/skills/*`
  - `dot_claude/skills/*`
  - `dot_codex/skills/*`（generate 後に `chezmoi re-add`）
  - 例: `browser-ops` と `browser-gherkin` は CLI-only browser automation の shared skill source
...
```sh
mise install npm:rulesync@7.15.2
```
...
この generate は `~/.claude`、`~/.codex`、`~/.gemini`、`~/.github` へ直接書き出します。
その後、chezmoi re-add で chezmoi source へ反映してください。

```sh
chezmoi re-add ~/.claude/CLAUDE.md ~/.claude/skills ~/.codex/AGENTS.md ~/.codex/skills ~/.gemini/GEMINI.md ~/.gemini/commands ~/.github/copilot-instructions.md
```
...
- browser automation 系 skill は source に成功手順だけを残し、探索ログや一時的な失敗例は含めないでください。
