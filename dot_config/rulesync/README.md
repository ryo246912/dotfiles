...
└── dot_config/rulesync/
    ...
    └── .rulesync/
        ...
        │   └── COPILOT.md  # Copilot instructions の source（repo では `.github/copilot-instructions.md` へ同期）
        ├── skills/
        │   └── <skill>/SKILL.md
        └── commands/
            └── <command>.md
...
- `dot_config/rulesync/.rulesync/rules/COPILOT.md`
  ...
  - repo ではこの内容を `.github/copilot-instructions.md` に同期して使う
...
Copilot だけは `rulesync` current target が `~/.copilot/copilot-instructions.md` を生成します。RTK 公式の hook/instructions は project-scoped の `.github/` を使うため、この repo では generated content を確認したうえで `.github/copilot-instructions.md` に同期してください。
