...
- ユーザーが PR 作成まで依頼した場合、PR を作って終了せず `pr-review-loop` skill を使用する
- CI と未解決レビュー指摘を定期的に確認し、修正、テスト、コミット、`origin` の同じブランチへの push、必要な返信を自律的に繰り返す
- 明示的な判断が必要な仕様変更、外部 CI の障害、認証切れ以外では、各修正前のユーザー確認を挟まない
