...
# identity列（GENERATED ALWAYS AS IDENTITY）へ明示idを入れるINSERTは、そのままでは
# `cannot insert into column` で止まる。SQL標準の OVERRIDING SYSTEM VALUE は
# CockroachDBが解釈しないため（`at or near "overriding": syntax error`）、取り込み前に
# 列をBY DEFAULTへ緩める。
#
...
relax_identity_columns() {
  ...
  while IFS='|' read -r table column; do
    [ -n "$table" ] || continue
    if ! psql_local --quiet --output=/dev/null \
      --command="ALTER TABLE \"${schema}\".\"${table}\"
        ALTER COLUMN \"${column}\" SET GENERATED BY DEFAULT"; then
